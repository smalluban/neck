...
Neck.Router = (function(_super) {
  ...
  function Router(opts) {
    ...
    this.yields = opts.yields;
  }

  Router.prototype.route = function(route, settings, callback) {
    var myCallback,
      _this = this;
    ...
    myCallback = function() {
      var args, options, query, yieldName, _ref, _results;
      ...
      if (args.length && !_.isRegExp(route)) {
        route.replace(_this.PARAM_REGEXP, function(all, name) {
          return query[name] = args.shift();
        });
      }
      _results = [];
      for (yieldName in settings) {
        options = settings[yieldName];
        options.params || (options.params = {});
        _results.push((_ref = _this.yields[yieldName]) != null ? _ref.append(options.controller || options, _.extend({}, options.params, query), options.refresh, options.replace) : void 0);
      }
      return _results;
    };
  ...
  function App(opts) {
    ...
    if (this.routes) {
      this.once('render:after', function() {
        new Neck.Router({
          yields: _this._yieldList,
          routes: _this.routes
        });
        return Backbone.history.start(_this.pushState ? {
          pushState: true
        } : void 0);
      });
    }
...
Neck.Helper["yield"] = (function(_super) {
  ...
  _yield.prototype.attributes = ['yieldView', 'yieldParams', 'yieldReplace'];

...
Neck.Controller = (function(_super) {
  ...
  Controller.prototype._setAccessor = function(key, value, controller) {
    ...
    if (controller !== this) {
      ...
      if (!this.scope._resolves[key].length) {
        return delete this.scope._resolves[key];
      }
    }
  };

  Controller.prototype._watch = function(key, callback, context) {
    var controller, resolve, val, _i, _len, _ref, _ref1, _results,
      _this = this;
    if (context == null) {
      context = this;
    }
  ...
  Controller.prototype.apply = function(key) {
    var resolve, _i, _len, _ref, _results;
    if (this.scope._resolves[key]) {
      _ref = this.scope._resolves[key];
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        resolve = _ref[_i];
        _results.push(resolve.controller.trigger("refresh:" + resolve.key));
      }
      return _results;
    } else {
...
Neck.Helper.list = (function(_super) {
  ...
  function list() {
    ...
    this.watch('listSort', function(sort) {
      if (sort) {
        return this.list = _.sortBy(this.list, function(i) {
          return sort(i.item);
        });
      }
    });
